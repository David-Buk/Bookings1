...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public List<Bookings> getAllBookings() {
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM Bookings ORDER BY date DESC", null);
    ...
    public List<Bookings> getBookingsByEmail(String email) {
        ...
        Cursor cursor = db.query(TABLE_BOOKINGS, columns, selection, selectionArgs, null, null, COLUMN_DATE);
    ...
    public List<Bookings> getBookingsByCampus(String campus) {
        ...
        Cursor cursor = db.query(TABLE_BOOKINGS, null, selection, selectionArgs, null, null, COLUMN_DATE);
    ...
    @SuppressLint("Range")
    public int getBookingId(String email, String date, String time) {
        SQLiteDatabase db = this.getReadableDatabase();
        String query = "SELECT id FROM bookings WHERE email = ? AND date = ? AND time = ?";
        Cursor cursor = db.rawQuery(query, new String[]{email, date, time});

        int bookingId = -1;
        if (cursor.moveToFirst()) {
            bookingId = cursor.getInt(cursor.getColumnIndex("id"));
        }
        cursor.close();
        db.close();
